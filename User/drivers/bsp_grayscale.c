...
static void get_grayscale_entry(void* parameter)
{
    while(1)
    {
        ...
        switch(Color)
        {
            ...
            case Front_Back_WhiteGreen:       //双灰度向前循迹
...
//函数功能：获得ch通道的AD值
//ch:通道值 ADC1_ch5和ADC2_ch8
uint16_t Get_Adc(uint8_t ch)
{
//    LED1=!LED1;
    //设置指定ADC的规则组通道，一个序列，采样时间
    ADC_RegularChannelConfig(ADC1, ch, 1, ADC_SampleTime_56Cycles );	         //ADC1,ADC通道,15个周期,提高采样时间可以提高精确度			    
...
//8
    A=1;B=1;C=1;
    rt_hw_us_delay(1);
    grayscale[7]=Get_Adc_Average(5,1);//获取通道5的转换值，3次取平
//7
    A=0;B=1;C=1;
    rt_hw_us_delay(1);
