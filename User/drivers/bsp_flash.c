...
void SaveSystemParam(uint8_t type)
{
	...
	switch(type)
	{
		case F_Grayscale :
				for(i=0;i<8;i++)
				{
					FrontGrayscale[i]=(F_White[i]+F_Green[i])/2;
				}
				for(i=0;i<8;i++)
				{
					FrontGrayscale[i+8]=(F_White[i]+F_Black[i])/2;
				   
				}
				for(i=0;i<8;i++)
				{
					FrontGrayscale[i+16]=(F_Green[i]+F_Black[i])/2;
				}	
				Flash_EraseSector(FLASH_Sector_9);
				Flash_Write16BitDatas(FrontGrayscaleAddr, 24, FrontGrayscale);	
             break;
				
		case B_Grayscale :
				for(i=0;i<8;i++)
				{
					BackGrayscale[i]=(B_White[i]+B_Green[i])/2;
				}
				for(i=0;i<8;i++)
				{
					BackGrayscale[i+8]=(B_White[i]+B_Black[i])/2;
				}
				for(i=0;i<8;i++)
				{
					BackGrayscale[i+16]=(B_Green[i]+B_Black[i])/2;
				}	
				Flash_EraseSector(FLASH_Sector_10);
				Flash_Write16BitDatas(BackGrayscaleAddr, 24, BackGrayscale);	
             break;		
        default:
             break;			
